...
### 3.3 新規追加が必要なモジュール
- **プレゼンモード (`presentation_mode/`)**: JSON台本＋固定左パネル＋右側キャラクター画像を組み合わせたプレゼン型タイムライン。Pollinations背景を一定秒数ごとに切り替え、ASS字幕を生成し、FFmpegでシーン毎に書き出してから連結するMVPを優先実装する。将来的に既存ロングパイプラインとの統合も検討する。
- シーンJSONでは `narration`（音声全文）と `subtitle_lines`（表示用チャンク）の両方を受け入れ、VOICEVOXの音素長から字幕タイミングを推定する。
